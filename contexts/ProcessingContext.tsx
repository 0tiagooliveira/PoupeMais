...
export const ProcessingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startProcessing = useCallback(async (mode: InputMode, file: File | null, textInput: string) => {
    ...
    try {
      ...
      4. CATEGORIZAÇÃO: Tente mapear para uma destas categorias padrão: Alimentação, Transporte, Moradia, Lazer, Saúde, Educação, Compras, Salário, Investimentos, Serviços, Outros.
      5. IMPORTANTE: Ignore pagamentos de fatura de cartão de crédito (para evitar duplicidade) e transferências entre contas do mesmo titular se detectado.
      ...
      if (extracted.length === 0) {
        ...
        // Marca todas como selecionadas por padrão
