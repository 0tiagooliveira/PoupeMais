...
export const NewTransactionModal: React.FC<NewTransactionModalProps> = ({ 
  ...
  useEffect(() => {
    if (isOpen) {
      if (transactionToEdit) {
        ...
        setRepeatCount('12');
        if (startOcrOnOpen) {
          // Delay to ensure rendering and avoid interference with modal transition
          setTimeout(() => {
  ...
  const handleScanReceipt = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsScanning(true);
    addNotification('Analisando documento...', 'info', 3000, true);
...
Apenas o JSON válido, sem NADA a mais (nem markdown, nem formatação). Se o recibo estiver ilegível, tente o seu melhor.`;
      ...
      if (extracted.type) setType(extracted.type as TransactionType);
      if (extracted.category) setCategory(extracted.category);

      addNotification('Nota escaneada com sucesso!', 'success');
    } catch (err) {
      console.error("OCR Scan Error:", err);
      addNotification('Falha ao ler o recibo. Tente novamente.', 'error');
    } finally {
      setIsScanning(false);
        ...
        {!transactionToEdit && (
           <div className="px-1 text-center">
             ...
             <button 
                ...
                disabled={isScanning}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-indigo-50/50 border border-indigo-100/50 text-indigo-600 font-bold text-xs hover:bg-indigo-50 hover:border-indigo-200 transition-all active:scale-95 disabled:opacity-50"
             >
                <div className={`flex items-center justify-center ${isScanning ? 'animate-spin' : ''}`}>
                   <span className="material-symbols-outlined text-lg">{isScanning ? 'sync' : 'document_scanner'}</span>
                </div>
                {isScanning ? 'Poup+ IA Lendo Recibo...' : 'Escanear Recibo com IA'}
                {!isScanning && <span className="ml-1 text-[9px] bg-indigo-100 px-1.5 py-0.5 rounded text-indigo-700 uppercase font-black uppercase tracking-widest">Novo</span>}
             </button>
           </div>
        )}
