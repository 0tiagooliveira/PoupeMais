...
export const Sidebar: React.FC = () => {
  ...
  const links = [
    ...
    { to: '/transactions', icon: 'payments', label: 'Transações' },
    { to: '/automation-rules', icon: 'auto_fix_high', label: 'Regras Inteligentes' },
  ...
  return (
    <aside className="hidden w-64 flex-col border-r border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 md:flex transition-colors duration-300">
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <div className={`flex h-8 w-8 items-center justify-center rounded-xl transition-all ${isActive ? 'bg-success/10 text-success' : 'text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-200'}`}>
